...
public:
    ...
    bool render(wgpu::Texture& texture) const;
...
bool FileRenderer::render(wgpu::Texture& texture) const {
    using namespace wgpu;
    auto device = m_device;
    auto width = m_width;
    auto height = m_height;
    ...
    auto callbackHandle = pixelBuffer.mapAsync(MapMode::Read, 0, pixelBufferDesc.size, [&](BufferMapAsyncStatus status) {
        ...
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                uint32_t i = y * width * 4 + x * 4;
                // Only compares R-channel
                if (pixelData[i] > 128) std::cout << "1";
                else std::cout << "0";
            }
            std::cout << std::endl;
        }
...
bool coutTexture(wgpu::Device& device, wgpu::Texture& texture) {
    using namespace wgpu;
    uint32_t width = texture.getWidth();
    uint32_t height = texture.getHeight();
